...
if (present(composer)) then
	write(*,"(A,A,A)") '  composer = "', TRIM(composer), '"'
	write(11,"(A,A,A)") '  composer = "', TRIM(composer), '"'
endif

if (present(subtitle)) then
	write(*,"(A,A,A)") '  subtitle = "', TRIM(subtitle), '"'
	write(11,"(A,A,A)") '  subtitle = "', TRIM(subtitle), '"'
endif

if (present(dedication)) then
	write(*,"(A,A,A)") '  dedication = "', TRIM(dedication), '"'
	write(11,"(A,A,A)") '  dedication = "', TRIM(dedication), '"'
endif
