subroutine HIDE(string,undo,once)
...
if (undo_AUX) then
  write(*,"(A,A)") "  \undo \hide ", TRIM(string)
  write(11,"(A,A)") "  \undo \hide ", TRIM(string)
  else
    if (once_AUX) then
      write(*,"(A,A)") "  \once \hide ", TRIM(string)
      write(11,"(A,A)") "  \once \hide ", TRIM(string)
      else
        write(*,"(A,A)") "  \hide ", TRIM(string)
        write(11,"(A,A)") "  \hide ", TRIM(string)
    endif
endif
